...
router.get('/cadastrar', loginCtrl.verificarAutenticacao, (req, res) =>
    res.render('cadastrar', { usuario: req.session.usuario }));

router.post('/cadastrar', (req, res) => {

    const { email, nome, senha } = req.body;
    const resposta1 = loginCtrl.registrarUsuario(email, nome, senha);
    res.render('cadastrar', { mensagem: resposta1.mensagem, usuario: req.session.usuario });
});
